...
async def set_mode(request: ModeRequest) -> ModeResponse:
    global _use_question_bank
    if _use_question_bank != request.use_question_bank:
        _answer_cache.clear()
    _use_question_bank = request.use_question_bank
    log.info("Question-bank map mode: %s", "on" if _use_question_bank else "off")
    return ModeResponse(use_question_bank=_use_question_bank)
...
async def ask(request: Request) -> AskResponse:
    ...
    cache_key = repr((use_question_bank, question, sorted(options.items())))
    ...
    if len(store) == 0:
        ...
        response = AskResponse(answer=answer, sources=[])
        _answer_cache[cache_key] = response
        return response
