...
def _set_local_map_mode(enabled: bool) -> None:
    url = f"http://127.0.0.1:{config.PORT}/mode"
    response = httpx.post(url, json={"use_question_bank": enabled}, timeout=10)
    response.raise_for_status()
    status = "on" if enabled else "off"
    print(f"local map mode: {status}")


def evaluate(document_received: bool = False, map_mode: bool = False) -> None:
    _set_local_map_mode(map_mode)
...
def main() -> None:
    ...
    if args.action == "evaluate":
        evaluate(document_received=args.document_received, map_mode=args.map)
        return
    {
        "register": register,
        "reset": reset,
